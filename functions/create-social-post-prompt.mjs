...
export const handler = async (state) => {
  try {
    const prompt = `Generate up to 5 social media posts for twitter and LinkedIn for an upcoming ` +
      ...
      `event JSON and use it to build engaging posts. Be upeat, use minimal emojis, and provide a ` +
      ...
      `today's date. Do not tag participants with @ mentions, only use their name. Create the posts ` +
      `with relevant tone for both Twitter and LinkedIn respectively. Use the startTime and timezone ` +
